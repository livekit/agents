...
class GenerationCreatedEvent:
    ...
    provider_function_calls: list[FunctionCall] = field(default_factory=list)
    """Tool calls the provider executed itself (e.g. xAI WebSearch). Populated
    by the realtime session as the response stream is parsed; read once at the
    end of the turn to emit ``function_tools_called``."""
