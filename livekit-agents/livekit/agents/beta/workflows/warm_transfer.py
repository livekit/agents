...
# the connector has no answered signal, so cap the wait to avoid hanging the caller
...
class TwilioConnectorWarmTransferTask(WarmTransferTask):
    def __init__(
        ...
        *,
        ...
        self._twilio_phone_number = phone_number
    ...
    async def _originate_human_agent(
        ...
        twiml = f"<Response><Connect><Stream url='{resp.connect_url}'/></Connect></Response>"

        client = Client(self._twilio_account_sid, self._twilio_auth_token)
        await asyncio.to_thread(
            client.calls.create,
            to=self._twilio_phone_number,
