...
class AssemblyaiOptions(TypedDict, total=False):
    ...
    agent_context: str  # free-text context to bias recognition (u3-rt-pro family only, max 1500 chars)
    voice_focus: Literal["near-field", "far-field"]  # isolate primary voice (u3-rt-pro family only)
    voice_focus_threshold: float  # how aggressively background audio is suppressed (u3-rt-pro family only)
    mode: Literal["min_latency", "balanced", "max_accuracy"]  # accuracy/latency preset (u3-rt-pro family only)
