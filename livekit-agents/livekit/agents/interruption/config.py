    ...
    def from_env(cls) -> 'InterruptionConfig':
        ...
        return cls(
            english_fillers=os.getenv(
                'INTERRUPTION_ENGLISH_FILLERS',
                'uh,um,umm,hmm,ah,er,mm'
            ).split(','),
            hindi_fillers=os.getenv(
                'INTERRUPTION_HINDI_FILLERS',
                'haan,han,ha,achha,theek'
            ).split(','),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--config-file',
        default='interruption_config.yaml',
        help='Path to configuration file'
    )
    parser.add_argument(
        '--test',
        action='store_true',
        help='Run quick configuration test'
    )
