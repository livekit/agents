...
class IntelligentInterruptionHandler:
    ...
    def _classify_interruption(
        self, text: str, confidence: float, agent_speaking: bool
    ) -> InterruptionType:
        """
    ...
    async def process_transcript(self, text: str, confidence: float, is_final: bool = True) -> bool:
        """
        ...
        async with self._lock:
            classification = self._classify_interruption(
                text=text, confidence=confidence, agent_speaking=self._agent_speaking
            )
