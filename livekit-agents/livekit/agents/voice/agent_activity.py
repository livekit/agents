...
class AgentActivity(RecognitionHooks):
    ...
    def min_endpointing_delay(self) -> float:
        return (
            ...
            else self._session.options.endpointing["min_delay"]
        )
    ...
    def max_endpointing_delay(self) -> float:
        return (
            ...
            else self._session.options.endpointing["max_delay"]
        )
    ...
    async def _start_session(self) -> None:
        ...
        if isinstance(self._interruption_detector, inference.AdaptiveInterruptionDetector):
            ...
            self._interruption_detector.on(
                "user_backchannel_detected", self._on_overlap_speech_ended
            )
    ...
    def _on_overlap_speech_ended(self, ev: inference.OverlappingSpeechEvent) -> None:
        if ev.is_interruption:
            ...
            self._session.emit("user_backchannel_detected", ev)
