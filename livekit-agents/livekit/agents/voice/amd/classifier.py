...
class _AMDClassifier(EventEmitter[Literal["amd_prediction"]]):
    ...
    def cancel_detection_timer(self) -> None:
        """Disarm the overall detection-timeout budget.

        Used when the budget was armed to bound an earlier phase that has now
        ended, and the detection window itself has not started yet.
        """
        if self._detection_timeout_timer is not None:
            self._detection_timeout_timer.cancel()
    ...
    def _on_timeout(
        ...
        Not gated by ``_listening_guard``: the publication-bound detection
        timeout can fire before listening begins.
        """
        if self._closed:
            return
