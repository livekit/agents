...
class KFLAPI:
    ...
    async def __aenter__(self) -> KFLAPI:
    ...
    async def _post(
                ...
                try:
                    async with session.post(
                        ...
                        if not response.ok:
                            ...
                            raise APIStatusError(
                                f"KFL API error for {url}: {response.status}",
                    ...
                    if isinstance(e, APIStatusError) and not e.retryable:
                        raise APIConnectionError(
                            f"Failed to call KFL API at {url} with non-retryable error",
                            retryable=False,
                        ) from e

                    if isinstance(e, APIConnectionError):
                        logger.warning("Failed to call KFL API", extra={"error": str(e)})
                    else:
                        logger.exception("Failed to call KFL API")
        ...
        raise APIConnectionError("Failed to call KFL API after all retries.")
