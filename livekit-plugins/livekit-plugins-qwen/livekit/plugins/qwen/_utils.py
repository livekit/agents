...
# Exempt from the plaintext warning: local proxies and tests legitimately use
# ws://127.0.0.1, and nothing leaves the machine there.
...
def resolve_realtime_url(base_url: str | None, region: QwenRegion) -> str:
    ...
    A plaintext ``ws://`` URL to a non-loopback host is allowed but logged: the API key
    travels in an ``Authorization`` header and the audio is unencrypted.
    """
    url = base_url or REALTIME_BASE_URLS[region]
    if url.startswith("ws://") and urlparse(url).hostname not in _LOOPBACK_HOSTS:
        logger.warning(
            "qwen base_url is plaintext ws://; the API key and all audio will be sent "
            "unencrypted. Use wss:// for any non-local host.",
            extra={"base_url": url},
        )
    return url
