...
from .models import FluxTTSModels

# Flux TTS is served on the /v2/speak endpoint over both WebSocket (streaming) and
# REST (batch). A single https base URL works for both: `_to_deepgram_url` flips the
# scheme to wss when a WebSocket connection is requested.
...
class TTSv2(tts.TTS):
    def __init__(
        self,
        *,
        model: FluxTTSModels | str = "flux-alexis-en",
        ...
        Args:
            model (FluxTTSModels | str): Flux TTS model to use. Defaults to "flux-alexis-en".
    ...
    async def _connect_ws(self, timeout: float) -> aiohttp.ClientWebSocketResponse:
        ...
        logger.debug(
            "Established new Deepgram Flux TTS WebSocket connection:",
    ...
    async def _close_ws(self, ws: aiohttp.ClientWebSocketResponse) -> None:
        try:
            ...
            await ws.send_str(SynthesizeStreamv2._FLUSH_MSG)
            await ws.send_str(SynthesizeStreamv2._CLOSE_MSG)
    ...
    def update_options(
        self,
        *,
        model: NotGivenOr[FluxTTSModels | str] = NOT_GIVEN,
        ...
        """
        Args:
            model (FluxTTSModels | str): Flux TTS model to use.
    ...
    def synthesize(
        ...
        return ChunkedStreamv2(tts=self, input_text=text, conn_options=conn_options)

    def stream(
        self, *, conn_options: APIConnectOptions = DEFAULT_API_CONNECT_OPTIONS
    ) -> SynthesizeStreamv2:
        stream = SynthesizeStreamv2(tts=self, conn_options=conn_options)
...
class ChunkedStreamv2(tts.ChunkedStream):
    def __init__(self, *, tts: TTSv2, input_text: str, conn_options: APIConnectOptions) -> None:
...
class SynthesizeStreamv2(tts.SynthesizeStream):
    _FLUSH_MSG: str = json.dumps({"type": "Flush"})
    _CLOSE_MSG: str = json.dumps({"type": "Close"})

    def __init__(self, *, tts: TTSv2, conn_options: APIConnectOptions):
