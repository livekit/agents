...
from .models import DEFAULT_SAMPLE_RATE, DEFAULT_VOICE_ID, VoiceID
...
class TTS(tts.TTS):
    ...
    Args:
        ...
        steps: Diffusion steps — the latency/quality dial. Higher values
            improve quality but increase latency (8 ultra-fast, 16 fast,
            32 balanced, 64 highest quality). Defaults to ``5`` for the
            lowest possible latency in a conversational agent.
    ...
    Note:
        Lokutor's TTS endpoint always returns audio at 44.1 kHz; the output
        sample rate is fixed and not configurable. The LiveKit pipeline
        resamples downstream if a different rate is required.
    """
    ...
    async def _connect_ws(self, timeout: float) -> aiohttp.ClientWebSocketResponse:
        ...
        # Send the API key in a header rather than the URL query string so the
        # credential does not end up in exception messages, tracing, or
                    ...
                    if msg.type == aiohttp.WSMsgType.BINARY:
                        ...
                        try:
                            ...
                            if isinstance(data, dict) and data.get("type") == "error":
                                # Don't interpolate the provider's raw message
                                # (it may echo customer content) into the error.
                                raise APIError("Lokutor returned an error response")
                        except json.JSONDecodeError:
                            logger.debug("Ignoring unexpected non-JSON text frame from Lokutor")

                output_emitter.flush()
        except asyncio.TimeoutError:
            raise APITimeoutError() from None
        except APIError:
            # Preserve typed errors raised above (APIStatusError, provider
            # error) instead of collapsing them into APIConnectionError.
            raise
...
class SynthesizeStream(tts.SynthesizeStream):
    ...
    async def _run(self, output_emitter: tts.AudioEmitter) -> None:
        ...
        try:
                    ...
                    while True:
                        ...
                        if msg.type == aiohttp.WSMsgType.BINARY:
                            ...
                            try:
                                data = json.loads(msg.data)
                                if isinstance(data, dict) and data.get("type") == "error":
                                    # Don't interpolate the provider's raw
                                    # message (may echo customer content).
                                    raise APIError("Lokutor returned an error response")
                            except json.JSONDecodeError:
                                logger.debug("Ignoring unexpected non-JSON text frame from Lokutor")
            ...
            # Preserve typed errors raised above instead of collapsing them
