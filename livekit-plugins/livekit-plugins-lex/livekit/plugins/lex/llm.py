...
class LLM(llm.LLM):
    ...
    def __init__(
        ...
        *,
        ...
        logger.info(
            "Lex V2 LLM initialized (bot=%s, alias=%s, locale=%s, region=%s)",
            bot_id,
            bot_alias_id,
            locale_id,
            resolved_region,
...
class LexLLMStream(llm.LLMStream):
    ...
    def _handle_client_error(self, error: Exception, error_code: str, request_id: str) -> None:
        """Map Lex ClientError codes to appropriate API errors."""
        body = str(error)
