...
class TTSPlugin:
    ...
    async def _push_data_loop(
        self, ws_wrapper: _WSWrapper, text_stream: AsyncIterable[str]
    ):
    ...
    async def _receive_audio_loop(
        self, ws_wrapper: _WSWrapper, result_queue: asyncio.Queue[rtc.AudioFrame]
    ):
        await ws_wrapper.wait_for_connected()
    ...
    def _create_frame_from_chunk(self, chunk: bytes):
        frame = rtc.AudioFrame.create(
            sample_rate=44100, num_channels=1, samples_per_channel=441
        )  # Eleven labs format
