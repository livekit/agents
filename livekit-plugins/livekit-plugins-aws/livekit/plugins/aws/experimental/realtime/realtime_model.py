...
class Boto3CredentialsResolver(IdentityResolver):  # type: ignore[misc]
    ...
    async def get_identity(self, **kwargs: Any) -> AWSCredentialsIdentity:
        ...
        if self._cached_identity:
            # Static credentials: _cached_expiry is None, cache indefinitely
            # Temporary credentials: check if still valid (5 min buffer before expiry)
            if self._cached_expiry is None or time.time() < self._cached_expiry:
        ...
        try:
            ...
            # Check if we just refreshed but got the same expired credentials
            if self._cached_identity and expiry_time:
                if self._cached_identity.access_key_id == identity.access_key_id:
                    # Same credentials after refresh attempt - they won't auto-refresh
                    if time.time() >= expiry_time.timestamp() - 300:
                        logger.warning(
                            "AWS credentials are expired or expiring and cannot be automatically refreshed. "
                            "Please update your credentials using 'aws sso login' or refresh your environment variables."
                        )
                        # Set expiry to None to stop trying to refresh
