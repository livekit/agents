...
# Muga requires exactly one global tone per utterance. When the caller sets no ``tone`` and
# the text carries no tag, we fall back to this neutral tone rather than failing synthesis.
...
class TTS(tts.TTS):
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            tone: Optional Muga fallback tone for untagged input. When omitted, untagged
                Muga input defaults to ``[neutral]``; a tone tag already present in the text
                always takes precedence. When provided, untagged input is prefixed with this
                tone, and any existing tag must match it.
    ...
    async def _stream_synthesis(
        ...
        *,
        ...
        """
        # Validate and build the payload BEFORE the network try-block below: a bad prompt is
        # a client-side error (ValueError), not a connection failure, so it must surface with
        # its real message and must never be wrapped as APIConnectionError or retried.
...
def _prepare_muga_text(text: str, opts: _TTSOptions) -> str:
    ...
    if match:
        ...
        # Untagged text: use the configured fallback tone, or DEFAULT_MUGA_TONE. A text tag
        # (handled above) always takes precedence over both.
        text = f"[{tone or DEFAULT_MUGA_TONE}] {text}"

    # Unknown event tags are still rejected, but any supported event is allowed with any
    # tone -- the tone/event pairing recommendations are prompting guidance, not hard rules,
    # so a "non-recommended" combination is passed through instead of failing synthesis.
...
def _prepare_mulberry_text(text: str) -> str:
    ...
    # Inline event tags (e.g. <laugh>) are a Muga feature, not a Mulberry one, but they are
    # passed through as-is rather than rejected: an LLM occasionally emitting one must not
    # crash synthesis.
    return text
