...
1. A request that is refused because capacity is busy is retried once so the
   turn stays alive. `await tts.prewarm()` is exposed if you would rather
   open the path on the SIP invite than on first speech.
...
#: The overflow lane answers with this while it opens. Not an error.
_COLD = "upstream_cold"
...
class GandrTTS(tts.TTS):
    ...
    Args:
        ...
        extra:       merged into every request body, `pronunciation_dict`,
                     ...
                     floor.
        base_url:    leave as-is; requests route to the nearest healthy
                     region (US + EU) automatically.
    ...
    def __init__(
        ...
        *,
        ...
        """`prewarm_on_start` wakes a worker as soon as the plugin is
...
class ChunkedStream(tts.ChunkedStream):
    ...
    async def _attempt(self, output_emitter: tts.AudioEmitter) -> None:
        ...
        try:
            async with self._tts._ensure_session().post(
                ...
                if resp.status >= 400:
                    ...
                    if resp.status in (502, 503) and _COLD in detail:
                        raise _ColdWorker()
                    raise APIStatusError(
        ...
        except (_ColdWorker, APIStatusError):
            raise
...
class _ColdWorker(Exception):
    """Internal: the fleet was asleep. Not surfaced to callers."""
