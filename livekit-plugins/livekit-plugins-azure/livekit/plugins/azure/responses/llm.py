...
class LLM(openai.responses.LLM):
    def __init__(
        self,
        *,
        model: str | ResponsesModel | None = None,
        ...
        if model is not None:
            warnings.warn(
                "The `model` parameter is deprecated for Azure OpenAI and will be removed in a "
                "future version. Azure uses `azure_deployment` for deployment selection.",
                DeprecationWarning,
                stacklevel=2,
            )
