...
from .models import InyaAILanguages
...
class STT(stt.STT):
    def __init__(
        self,
        *,
        language: InyaAILanguages | str = "en-IN",
        ...
        """Create a new instance of InyaAI (Gnani) STT.

        Args:
            language: Target transcription language code (e.g., "en-IN", "hi-IN").
                     Defaults to "en-IN".
            api_key: Your InyaAI API key. If not provided, will look for INYAAI_API_KEY
                    environment variable.
            organization_id: Organization ID for the API. If not provided, will look for
                           INYAAI_ORG_ID environment variable.
            user_id: User ID for the API. If not provided, will look for INYAAI_USER_ID
                    environment variable.
            base_url: The base URL for InyaAI API. Defaults to "https://api.vachana.ai/stt/v3".
    ...
    def update_options(self, *, language: InyaAILanguages | str | None = None) -> None:
        """Update STT options.
    ...
    async def _recognize_impl(
        ...
        *,
        ...
        """Recognize speech from audio buffer using InyaAI API.
        ...
        try:
            ...
            async with session.post(
                ...
                if response.status != 200:
                    ...
                    logger.error(f"InyaAI API error ({response.status}): {error_text}")
                    raise APIStatusError(
                        message=f"InyaAI API returned status {response.status}",
                ...
                if not result.get("success", False):
                    ...
                    logger.error(f"InyaAI API returned failure: {error_msg}")
                    raise APIConnectionError(f"InyaAI API error: {error_msg}")
            ...
            logger.error(f"InyaAI API response error: {e.status} {e.message}")
            raise APIStatusError(
                message=e.message,
            ...
            logger.error(f"InyaAI connection error: {e}")
            raise APIConnectionError(f"InyaAI connection error: {e}") from e
        except Exception as e:
            logger.exception(f"Unexpected error during InyaAI recognition: {e}")
        ...
        return stt.SpeechEvent(
            ...
            alternatives=[
                stt.SpeechData(
                    text=text,
                    language=language,
                    confidence=1.0,  # InyaAI API doesn't return confidence scores
                )
            ],
        )
