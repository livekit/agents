# LiveKit InyaAI Plugin

This plugin provides Speech-to-Text functionality for LiveKit Agents using InyaAI's (Gnani Vachana) multilingual STT engine with strong support for Indian languages.

## Installation

```bash
pip install livekit-plugins-inyaai
```
...
You'll need to set up your InyaAI credentials. You can do this via environment variables:

```bash
export INYAAI_API_KEY="your-api-key"
export INYAAI_ORG_ID="your-organization-id"
export INYAAI_USER_ID="your-user-id"
```
...
from livekit.plugins import inyaai

async def entrypoint(ctx: JobContext):
    # Initialize InyaAI STT
    stt_instance = inyaai.STT(
...
stt_instance = inyaai.STT(language="ta-IN")  # Tamil
...
The InyaAI STT plugin uses the Gnani Vachana API v3 endpoint:
...
- [InyaAI Vachana Platform](https://vachana.ai)
