...
class WizperSTT(stt.STT):
    def __init__(self, *, language='en', task='transcribe', chunk_level='segment', version='3', api_key=None):
        ...
        self._opts = _STTOptions(language=language, task=task, chunk_level=chunk_level, version=version)

        if not self._api_key:
            raise ValueError("FAL AI API key is required. It should be set with env FAL_KEY")

    def _sanitize_options(self, *, language: str = None, task: str = None, chunk_level: str = None, version: str = None) -> _STTOptions:
    ...
    async def recognize(self, buffer: AudioBuffer, *, language: str = None, task: str = None, chunk_level: str = None, version: str = None) -> stt.SpeechEvent:
        logger.debug("Starting recognition process.")
        try:
            if buffer is None:
                raise ValueError("AudioBuffer input is required")
            
            config = self._sanitize_options(language=language, task=task, chunk_level=chunk_level, version=version)
            ...
            try:
                result = await fal_client.result_async("fal-ai/wizper", handler.request_id)
    ...
    def _transcription_to_speech_event(self, event_type=SpeechEventType.FINAL_TRANSCRIPT, text=None) -> stt.SpeechEvent:
