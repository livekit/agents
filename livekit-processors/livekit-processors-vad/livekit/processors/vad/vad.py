...
class VAD:
    ...
    async def push_frame(self, frame: livekit.AudioFrame):
    ...
    def _process_frames(self, frames: [livekit.AudioFrame]) -> bool:
        ...
        for i, f in enumerate(resampled):
            self._copy_int16[i * len(f.data): (i + 1) * len(f.data)] = np.ctypeslib.as_array(f.data)
            self._copy_float32 = self._copy_int16.astype(np.float32, copy=False) / 32768.0
        ...
        self._window_buffer[:-buffer_count] = self._window_buffer_scratch[:-buffer_count]
