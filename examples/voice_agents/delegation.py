"""Fast brain / slow brain: a realtime model talks, an expert agent reasons.

The conversation model owns latency and turn-taking and gets one extra tool, `delegate`.
Everything that needs reasoning, lookups or actions goes to the fare desk, which never speaks
to the caller — it returns facts the conversation model phrases.

The desk declares where a delegation got to — `working` while it works, `completed` with the
answer — so nothing has to work out which of its outputs was the result.

It runs in this process, which is what lets the delegations of one conversation share a tool
executor: a later one can replace work an earlier one started. Ask to move a booking, change
your mind a moment later, and the first reissue is cancelled rather than left running.

`delegation_remote.py` delegates to an expert running somewhere else, over A2A.

    python delegation.py console
"""
...
bookings or tells you there is no address yet. You have no voice, so you cannot collect one:
answer "I need the caller's email address" and stop, and the phone agent will ask and come
back with the request again. Say that once and only when a tool has told you it is missing —
asking again in one call is the rudest thing you can do to them. A first-time caller needs an
address too — it is what book_flight opens their account with — but ask for it when they are
ready to buy, not while they are still looking.
...
# purely conversational, and deliberately not told who does the rest. calling the other half
# "the fare desk" here would invite it to decide a weather question is out of scope and answer
# from memory instead of delegating. `delegation_remote.py` puts the same voice in front of an
# expert reached over A2A
VOICE_INSTRUCTIONS = (
    ...
    "the caller for personal details of your own accord: when the work needs an "
    "email address it will say so, and that is when you run collect_email and "
...
class SupportAgent(Agent):
    """The voice. One extra tool beyond `delegate`, because only this side can be heard."""
    ...
    def __init__(self) -> None:
        ...
        super().__init__(
            instructions=f"Today is {date.today():%A, %Y-%m-%d}. " + VOICE_INSTRUCTIONS,
    ...
    async def collect_email(self, ctx: RunContext[Userdata], change: bool = False) -> str:
        ...
        it only once the desk has said it needs an address, never to open with, and
...
class FareDesk(Agent):
    """The expert. An ordinary agent, and the only thing that sees the tools."""

    def __init__(self) -> None:
        super().__init__(
            instructions=policy(date.today()),
    ...
    async def lookup_caller(self, ctx: RunContext[Userdata]) -> dict[str, Any]:
        ...
        if not userdata.email:
            raise ToolError("no address on file — ask the phone agent for one")
...
async def entrypoint(ctx: JobContext) -> None:
    ...
    # Both levels come through this one handler, so it reads as a trace of the hand-off:
    # what the voice model asked for, each step the desk took, and what went back. The desk
    # runs in its own session, so its calls are on that session's events and nowhere else —
    # they land in neither chat history.
    #
    ...
    def _on_tool_execution_updated(ev: ToolExecutionUpdatedEvent) -> None:
        ...
        if update.type == "tool_call_started":
            ...
            # the delegate tool's own updates are the desk's progress on its way to the
            # voice model, already logged under the tool that reported it. a silent one is
            # recorded for the model but never spoken — the dispatch note with `announce`
            # off — and is not worth a line either way
    ...
    def desk_session() -> AgentSession[Userdata]:
        """One per delegation, sharing the call's userdata so the address survives into it."""
        desk = AgentSession[Userdata](userdata=userdata, max_tool_steps=8)
        desk.on("tool_execution_updated", _on_tool_execution_updated)
        return desk

    session = AgentSession[Userdata](
        userdata=userdata,
        max_tool_steps=8,
        delegate=AgentDelegate(FareDesk, session_factory=desk_session),
        # the realtime model writes its own line alongside the call, so the dispatch note
        # asks for nothing and the acknowledgement costs no round trip
        delegation_options={"announce": False, "metadata": {"room": ctx.room.name}},
    )
