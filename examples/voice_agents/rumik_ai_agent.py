...
async def entrypoint(ctx: JobContext) -> None:
    ...
    def _on_metrics_collected(ev: MetricsCollectedEvent) -> None:
        # Logs per-turn latency for every component (STT, LLM, TTS, end-of-turn). The TTS
        # line carries the rumik-ai TTFB; we also surface it explicitly for quick reading.
        metrics.log_metrics(ev.metrics)
        if isinstance(ev.metrics, metrics.TTSMetrics):
            logger.info("rumik-ai TTS TTFB: %.0f ms (model=%s)", ev.metrics.ttfb * 1000, tts_model)
