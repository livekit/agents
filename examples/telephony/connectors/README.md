...
| `TWILIO_AUTH_TOKEN` | Auth token. Used by the `dial` command, and when set, inbound webhook signatures are verified |
| `TWILIO_FROM_NUMBER` | Your Twilio number, for the `dial` command only |

1. Run the server: `python twilio_connector.py serve`
...
| `WHATSAPP_VERIFY_TOKEN` | The token you choose when registering the webhook |
...
2. Run the server: `python whatsapp_connector.py serve`
...
Connector calls support the warm transfer workflow in [`../warm-transfer/`](../warm-transfer/). The customer leg needs no changes, no matter which connector brought the call in. There are two ways to reach the supervisor:
...
To try it with these examples, run the warm transfer agent instead of the DTMF agent, and point the webhook server at its dispatch name:
