...
from livekit.agents.pipeline.avatar import AvatarRunner, MediaOptions, VideoGenerator
    ...
    async def push_audio(self, frame: rtc.AudioFrame | AudioFlushSentinel) -> None:
        ...
        if isinstance(frame, rtc.AudioFrame):
            ...
            if self._audio_resampler is not None:
                ...
                # Send directly if no resampling is needed
                await self.simliClient.send(
                    frame.to_wav_bytes()[44:]
                )  # Remove wav header

        elif isinstance(frame, AudioFlushSentinel) and self._audio_resampler is not None:
            # flush the resampler
