...
def test_authentication(service: MockBankService) -> None:
    ...
    assert "53001422" in ids

    assert service.customer_exists("53001422")
    assert not service.customer_exists("00000000")

    assert service.authenticate("53001422", "1942")
    assert not service.authenticate("53001422", "9999")


def test_deposit_accounts(service: MockBankService) -> None:
    accounts = service.list_deposit_accounts("53001422")
    assert len(accounts) == 2
    checking = service.find_deposit_account("53001422", "031890246")
    assert checking is not None
    assert checking.account_type == "Checking"

    total = service.calculate_total_deposits("53001422")
    assert total == pytest.approx(4821.37 + 18250.41)


def test_credit_cards(service: MockBankService) -> None:
    cards = service.list_credit_cards("28890317")
    assert len(cards) == 1
    card = cards[0]
    assert card.minimum_due == pytest.approx(25.00)

    total_balance = service.calculate_total_card_balance("28890317")
    assert total_balance == pytest.approx(412.09)


def test_loans(service: MockBankService) -> None:
    loans = service.list_loans("28890317")
    assert len(loans) == 2
    assert loans[0].loan_type == "Auto Loan"

    outstanding = service.calculate_total_loan_balance("28890317")
    assert outstanding == pytest.approx(18642.77 + 19880.43)

    payments = service.upcoming_payments("28890317")
...
def test_rewards_summary(service: MockBankService) -> None:
    rewards = service.get_rewards("53001422")
...
def test_support_tickets(service: MockBankService) -> None:
    tickets = service.list_support_tickets("53001422")
    assert len(tickets) == 1
    assert tickets[0].status == "Awaiting Customer"

    no_tickets = service.list_support_tickets("28890317")
    assert no_tickets == ()
...
def test_format_helpers(service: MockBankService) -> None:
    ...
    account = service.find_deposit_account("53001422", "031890246")
