...
Before asking anything, scan the conversation so far. If dates, room type, party size, or smoking preference were already discussed, call the matching recording tools (set_stay, choose_room) right away with those values - don't re-ask the caller for details they already gave. When the conversation covered more than one room, only that room's values carry into this flow: ask the caller for this room's check-out date rather than reusing another room's. That scan covers the stay and the room; it is not a licence to skip a dialog.
