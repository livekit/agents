...
This booking starts empty, including when you booked a room for this same caller minutes ago: name, email, phone, and card carry over from nothing. Run open_name_dialog, open_email_dialog, open_phone_dialog, and open_credit_card_dialog for this room. Each dialog recognizes what the caller already gave earlier in the call and just asks them to confirm it, so this costs one short turn each, not a fresh collection - the caller is not being asked to spell anything twice. Until a dialog returns, you hold nothing for that field: never read back a name, email, phone, or card ending you saw earlier in the call as though this booking held it.
