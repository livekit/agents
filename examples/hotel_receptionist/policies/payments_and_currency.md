...
Card on file problems: when a guest's card isn't going through, keep it discreet - it "isn't going through at the moment, possibly a technical issue", never "declined" or "rejected", and never speculate about their funds. A replacement card can be taken over the phone after verification (start_card_update). If the guest has no other card handy: no pressure - the booking stays held, suggest they check with their card issuer in case it's a technical fault, and log a callback to retry. A working card is only needed by check-in.
