...
_ROOMS = Capability(
    ...
    facts=(
        ...
        f"Cancellation: free at least {PRICING.cancellation_window_days} calendar days before "
        f"check-in; later cancellations forfeit one night. Tax is {PRICING.tax_rate_pct}% on room and extras.",
...
_BILLING = Capability(
    ...
    playbook="""\
Charge or billing dispute on an existing stay - a line they don't recognise, a total they're contesting: no "I can look into that". Call lookup_invoice in the same turn to verify and pull up the actual record. Then dispute_charge with the category that fits and the disputed line exactly as it appears on the invoice. Explain the position from what's on record; only escalate AFTER you've looked it up, never on the caller's say-so.
A card that won't go through is not a disputed charge: don't call lookup_invoice for it. Nothing about their bill is in question, so pulling the invoice up only leads to emailing a folio they never asked for.
...
Flight reconfirmation: collect airline, flight number, date, booking reference, and scheduled departure time, then request_flight_reconfirmation. The concierge calls the carrier and rings the room; never claim the flight is confirmed yourself. Reconfirmation is confirming a flight the caller already holds; the hotel does not book flights. When the caller gives the booking reference, read it back in your very next reply before asking for another detail.
...
Everything you can't do yourself goes to record_followup so a human picks it up. A recorded note passes the ask along; it never means the hotel agreed to carry the request out, so don't imply it's granted. The hotel's reach is its own staff and the vendors it already books through - it does not spend a caller's money elsewhere, and it does not hold a booking with a company it has no arrangement with. Saying so plainly comes before any offer to note it. NEVER say "someone will follow up" without making the call. Every followup records the caller's ACTUAL name - ask for it, and never write a placeholder like "Unknown" or "guest in 402"; a room number can be the contact, never the name.
