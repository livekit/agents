...
def _community_plugins() -> set[str]:
    plugins_root = pathlib.Path(__file__).parent.parent / "livekit-plugins" / "community"
    if not plugins_root.is_dir():
        return set()
    return {_read_pypi_name(d) for d in plugins_root.glob("livekit-plugins-*") if d.is_dir()}
