...
It does not modify LiveKit’s internal VAD; it works purely through public events (agent\_speech\_started, agent\_speech\_ended, transcription) and calls `session.interrupt()` when needed.
...
| `classifier.py` | Classifies user utterances (LOW\_CONF, FILLER\_ONLY, HARD\_INTENT, CONTENT). |
...
2.  Fill your `LIVEKIT_*` and `OPENAI_API_KEY`
3.  Run:
    ```bash
