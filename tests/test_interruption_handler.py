...
class TestBasicFillerDetection:
    ...
    async def test_single_filler_when_speaking(self):
        ...
        assert not result, "Should not interrupt on single filler"
    ...
    async def test_multiple_fillers_when_speaking(self):
        ...
        assert not result, "Should not interrupt on multiple fillers"
    ...
    async def test_filler_when_quiet(self):
        ...
        result = await handler.process_transcript("umm", confidence=0.8)
        assert not result, "Should register but not interrupt when quiet"
...
class TestRealInterruptions:
    ...
    async def test_stop_command(self):
        ...
        assert result, "Should interrupt on 'stop' command"
    ...
    async def test_wait_command(self):
        ...
        assert result, "Should interrupt on 'wait' command"
    ...
    async def test_mixed_filler_and_command(self):
        ...
        assert result, "Should interrupt when command is present"
    ...
    async def test_sentence_with_filler(self):
        ...
        assert result, "Should interrupt on meaningful content"
...
class TestConfidenceThreshold:
    ...
    async def test_low_confidence_ignored(self):
        ...
        assert not result, "Should ignore low confidence speech"
    ...
    async def test_high_confidence_processed(self):
        ...
        assert result, "Should process high confidence speech"
    ...
    async def test_confidence_threshold_exact(self):
        ...
        assert result, "Should process speech at threshold"
...
class TestDynamicConfiguration:
    ...
    async def test_add_ignored_words(self):
        ...
        handler = IntelligentInterruptionHandler(ignored_words=["uh"], allow_runtime_updates=True)
        handler.set_agent_speaking(True)

        # Initially "okay" should interrupt
        result1 = await handler.process_transcript("okay", confidence=0.8)
        assert result1, "Should interrupt before 'okay' is added"
        ...
        assert not result2, "Should ignore after 'okay' is added"

    @pytest.mark.asyncio
    async def test_replace_ignored_words(self):
        """Should be able to replace entire ignored list"""
        handler = IntelligentInterruptionHandler(
            ignored_words=["uh", "um"], allow_runtime_updates=True
        )
        handler.set_agent_speaking(True)
        ...
        assert result1, "Should interrupt on previously ignored word"

        # New words should be ignored
        result2 = await handler.process_transcript("hmm", confidence=0.8)
        assert not result2, "Should ignore newly added word"

    @pytest.mark.asyncio
    async def test_updates_disabled(self):
        """Updates should be rejected when disabled"""
        handler = IntelligentInterruptionHandler(ignored_words=["uh"], allow_runtime_updates=False)
...
class TestEdgeCases:
    ...
    async def test_empty_transcript(self):
        ...
        assert not result, "Empty transcript should not interrupt"
    ...
    async def test_whitespace_only(self):
        ...
        result = await handler.process_transcript("   ", confidence=0.8)
        assert not result, "Whitespace should not interrupt"
    ...
    async def test_case_insensitive(self):
        ...
        assert not result1, "Should ignore uppercase filler"
        assert not result2, "Should ignore mixed case filler"
        assert not result3, "Should ignore mixed case filler"
    ...
    async def test_rapid_state_changes(self):
        ...
        for i in range(100):
            ...
            result = await handler.process_transcript(
                "uh" if i % 3 == 0 else "stop", confidence=0.8
            )
    ...
    async def test_concurrent_processing(self):
        ...
        tasks = [handler.process_transcript(f"test {i}", confidence=0.8) for i in range(50)]
...
class TestIntegration:
    ...
    async def test_background_noise_handling(self):
        ...
        assert not r1, "Should ignore low confidence background noise"

        # Clear speech (high confidence)
        r2 = await handler.process_transcript("stop", confidence=0.9)
        assert r2, "Should process clear speech"
...
class TestRobustness:
    ...
    async def test_rapid_speech_burst(self):
        ...
        # 50 rapid inputs
        tasks = [handler.process_transcript("test" if i % 3 else "uh", 0.8) for i in range(50)]
        results = await asyncio.gather(*tasks)
        assert len(results) == 50
