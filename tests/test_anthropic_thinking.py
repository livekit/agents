...
sys.path.insert(
    0, os.path.join(os.path.dirname(__file__), "..", "livekit-plugins", "livekit-plugins-anthropic")
)
...
def test_thinking_config_types():
    ...
    thinking_config: ThinkingConfig = {"type": "enabled", "budget_tokens": 5000}

    assert thinking_config["type"] == "enabled"
    assert thinking_config["budget_tokens"] == 5000

    # Test ThinkingConfigDict alias
    thinking_dict: ThinkingConfigDict = {"type": "enabled", "budget_tokens": 3000}
...
def test_llm_creation_with_thinking():
    ...
    llm_with_dict = LLM(thinking={"type": "enabled", "budget_tokens": 5000})
    ...
    thinking_config: ThinkingConfig = {"type": "enabled", "budget_tokens": 3000}
