...
def test_muga_untagged_text_defaults_to_neutral() -> None:
    # No fallback tone configured and no tag in the text -> default to [neutral] instead of
    # failing, so muga "just works" without wiring a tone.
    tts = rumik_tts.TTS(api_key="test-key")

    assert rumik_tts._prepare_text("Arre yaar", tts._opts) == "[neutral] Arre yaar"


@pytest.mark.asyncio
async def test_muga_invalid_prompt_raises_value_error_not_connection_error() -> None:
    # A bad prompt (Devanagari for muga) must surface as a plain ValueError with its real
    # message -- not be wrapped as an APIConnectionError and retried.
    tts = rumik_tts.TTS(api_key="test-key", model="muga")

    with pytest.raises(ValueError, match="Roman script"):
        async for _ in tts.synthesize("मैं ठीक हूँ"):
            pass
...
@pytest.mark.parametrize(
    ...
    [
        "Arre <laugh> yaar",
        "Theek hai <sigh>",
        "प्रियो जी <chuckle> कैसे हैं?",
    ],
)
def test_mulberry_allows_inline_event_tags(text: str) -> None:
    # Inline event tags are a Muga feature; Mulberry passes them through unchanged rather
    # than failing synthesis when an LLM happens to emit one.
    tts = rumik_tts.TTS(api_key="test-key", model="mulberry")
...
def test_muga_allows_non_recommended_event_tone_pairing() -> None:
    # Tone/event pairing recommendations are prompting guidance, not enforced: a supported
    # event with any tone passes through instead of being rejected.
    tts = rumik_tts.TTS(api_key="test-key", tone="sad")

    assert rumik_tts._prepare_text("[sad] <laugh> sab khatam", tts._opts) == (
        "[sad] <laugh> sab khatam"
    )
