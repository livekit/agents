...
def test_the_extension_names_itself_and_its_keys() -> None:
    ...
    assert a2a.DATABASE == "https://livekit.io/a2a/ext/agent-session/v1/database"
...
def test_the_persistence_keys_round_trip_beside_the_kind() -> None:
    task_input = a2a.TaskInput(
        instruction="find it", database_id="DB_abc", caller_session_id="voice"
    )
    ...
    assert (back.database_id, back.caller_session_id) == ("DB_abc", "voice")

    # a person's turn carries them too, and a request without them reads as None
    turn = a2a.to_a2a_request(a2a.TaskInput(text="hi", database_id="DB_abc"), context_id="s")
    assert as_dict(turn.message.metadata) == {a2a.DATABASE: "DB_abc"}
    plain = a2a.from_a2a_request(a2a.to_a2a_request(a2a.TaskInput(text="hi"), context_id="s"))
    assert (plain.database_id, plain.caller_session_id) == (None, None)
