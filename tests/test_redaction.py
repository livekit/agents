...
@pytest.mark.asyncio
async def test_redact_chat_ctx_redacts_function_call_arguments_for_non_llm_sinks() -> None:
    # the LLM can echo PII it saw into tool arguments (e.g. a card number passed
    # to a payment tool); at-rest sinks must scrub them
    chat_ctx = ChatContext.empty()
    chat_ctx.insert(
        FunctionCall(
            name="charge_card",
    ...
    for sink in (RedactionSink.TRANSCRIPT, RedactionSink.TELEMETRY):
        redacted = await redact_chat_ctx(chat_ctx, opts, sink=sink)
        assert "4242" not in redacted.items[0].arguments
        assert "[CREDIT_CARD]" in redacted.items[0].arguments

    assert chat_ctx.items[0].arguments == '{"card": "4242 4242 4242 4242"}'


@pytest.mark.asyncio
async def test_redact_chat_ctx_leaves_function_call_arguments_for_llm_sink() -> None:
    # LLM-generated arguments round-trip back to the model that produced them:
    # redacting them for the LLM sink would only corrupt the tool loop
    chat_ctx = ChatContext.empty()
    chat_ctx.insert(
        FunctionCall(
            name="charge_card",
    ...
    assert redacted.items[0].arguments == '{"card": "4242 4242 4242 4242"}'
