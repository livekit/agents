...
from livekit.agents.voice.amd._fsm import _AMDFSM, _Transcript
from livekit.agents.voice.amd.events import AMDCategory, AMDCompletedEvent
...
def test_timeout_resolves_reply_but_late_result_can_change_stage(fsm: _AMDFSM) -> None:
    ...
    assert fsm.authorize_reply(1) == AMDCategory.UNCERTAIN
    ...
    assert fsm.finished and fsm.authorize_reply(1) == AMDCategory.HUMAN
    fsm.tick(4)
...
def test_superseded_request_and_timer_cannot_change_newer_turn(fsm: _AMDFSM) -> None:
    ...
    assert fsm.authorize_reply(1) is None
    assert fsm.authorize_reply(second) is None
    fsm.prediction_received(second, AMDCategory.MACHINE_SCREENING, 2, 0.9)
    assert fsm.authorize_reply(second) == AMDCategory.MACHINE_SCREENING
...
def test_older_empty_transcript_cannot_reanchor_newer_silence() -> None:
    ...
    assert fsm.authorize_reply(old_turn) is None
    assert fsm.authorize_reply(current) == AMDCategory.MACHINE_SCREENING
...
def test_new_speech_waits_for_eot_and_empty_turn_reuses_held_prediction() -> None:
    ...
    assert fsm.authorize_reply(1) is None
    assert fsm.authorize_reply(second) == AMDCategory.MACHINE_VM
...
def test_late_transcript_updates_only_the_next_request(fsm: _AMDFSM) -> None:
    ...
    assert "alternative_transcript" not in second["earlier_turns"][0]
...
def test_finish_settles_pending_turns_and_ignores_late_work(fsm: _AMDFSM) -> None:
    request(fsm, 1)
    fsm.dtmf_sent("12#")
    fsm.finish("participant_disconnected")
